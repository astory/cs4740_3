...
public class History implements GuideUserHistory, GuideHistory {
	protected ArrayList<ActionDecision> history;
	...
	public History(String decisionSettings, String separator, HashMap<String, TableHandler> tableHandlers) throws MaltChainedException {
		...
		history = new ArrayList<ActionDecision>();
		clear();
	}
	...
	protected ActionDecision getEmptyActionObject() throws MaltChainedException {
		ActionDecision actionObject = null;
		if (currentAction + 1 >= history.size()) {
			actionObject = new ComplexDecisionAction(this);
		} else {
			actionObject = history.get(++currentAction);
			actionObject.clear();
		}
		return actionObject;
	}
