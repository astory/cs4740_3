...
@since 1.0
*/
public class SingleGuide implements Guide {
	...
	protected void initHistory() throws MaltChainedException {
		...
		int kBestSize = 1;
		if (guideMode == Guide.GuideMode.CLASSIFY) {
	...
	protected void initDecisionModel(SingleDecision decision) throws MaltChainedException {
		...
		try {
			Class<?>[] argTypes = { org.maltparser.parser.guide.Guide.class, org.maltparser.core.feature.FeatureModel.class };
	...
	protected void initFeatureModelManager() throws MaltChainedException {
		final FeatureEngine system = new FeatureEngine(configuration);
		system.load("/appdata/features/ParserFeatureSystem.xml");
		system.load(PluginLoader.instance());
		featureModelManager = new FeatureModelManager(system, configuration.getRegistry());
	}
	...
	protected void initFeatureModel() throws MaltChainedException {
		...
		featureModel = getFeatureModelManager().getFeatureModel(getConfiguration().getOptionValue("guide", "features").toString(), 0);
		if (getGuideMode() == Guide.GuideMode.TRAIN) {
			try {
