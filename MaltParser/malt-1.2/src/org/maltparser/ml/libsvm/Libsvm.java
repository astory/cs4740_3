...
public class Libsvm implements LearningMethod {
	public final static String LIBSVM_VERSION = "2.86";
	...
	//private FastMulticlassModel fastModel = null;
	/**
	...
	public void train(FeatureVector featureVector) throws MaltChainedException {
		...
		if (pathExternalSVMTrain == null) {
			...
			try {			
				final ArrayList<Integer> cardinalities = new ArrayList<Integer>();
				...
				final String errorMessage = svm.svm_check_parameter(prob, svmParam);
				...
				//System.setErr(new PrintStream(new LoggingOutputStream(owner.getGuide().getConfiguration().getConfigLogger(), owner.getGuide().getConfiguration().getConfigLogger().getLevel()), true));
	...
	public boolean predict(FeatureVector featureVector, SingleDecision decision) throws MaltChainedException {
		...
		for (FeatureFunction feature : featureVector) {
			...
			offset += featureValue.getCardinality();
		}
		...
		if (decision.getKBestList().getK() == 1 || svm.svm_get_svm_type(model) == svm_parameter.ONE_CLASS ||
			...
			decision.getKBestList().add((int)svm.svm_predict(model, xlist.subList(0, j).toArray(new svm_node[0])));
		} else {
			svm_predict_with_kbestlist(model, xlist.subList(0, j).toArray(new svm_node[0]), decision.getKBestList());
		}
	 ...
	 */
	public final void readProblemMaltSVMFormat(InputStreamReader isr, svm_problem prob, ArrayList<Integer> cardinality, svm_parameter param) throws MaltChainedException {
		try {
	...
	public void svm_predict_with_kbestlist(svm_model model, svm_node[] x, KBestList kBestList) throws MaltChainedException {
		...
		for (i=0;i<nr_class-1;i++) {
			...
			vote[small] = vote[i];
			vote[i] = temp;
			temp = voteindex[small];
			voteindex[small] = voteindex[i];
			voteindex[i] = temp;
		}
		...
		for (i=0; i<nr_class && k >= 0; i++, k--) {
			if (vote[i] > 0 || i == 0) {
				//kBestList.addKBestItem(labels[voteindex[i]], (double)vote[i]/(double)(nr_class*(nr_class-1)/2));
				//kBestList.addKBestItem(labels[voteindex[i]]);
	 ...
	 * @throws LibsvmException
	 */
	public static void maltSVMFormat2OriginalSVMFormat(InputStreamReader isr, OutputStreamWriter osw, ArrayList<Integer> cardinality) throws MaltChainedException {
		try {
