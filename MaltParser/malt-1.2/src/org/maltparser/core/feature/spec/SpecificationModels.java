...
public class SpecificationModels {
	...
	public void load(String specModelFileName) throws MaltChainedException {
		...
		if (url == null) {
			...
			if (indexSlash == -1) {
				url = Util.findURL(configuration.getConfigurationDir().getFile(specModelFileName).getAbsolutePath());
			} else {
				url = Util.findURL(configuration.getConfigurationDir().getFile(specModelFileName.substring(indexSlash+1)).getAbsolutePath());
			}
		}
		load(url);
	}
	...
	public SpecificationModel getSpecificationModel(String specModelURL, int specModelUrlIndex) throws MaltChainedException {
		...
		if (url == null) {
			...
			if (indexSlash == -1) {
				url = Util.findURL(configuration.getConfigurationDir().getFile(specModelURL).getAbsolutePath());
			} else {
				url = Util.findURL(configuration.getConfigurationDir().getFile(specModelURL.substring(indexSlash+1)).getAbsolutePath());
			}
		}
