...
for classifier in memm bayes tree; do
  for bootstrap in off on; do #Fix how a lack of bootstrapping for trees doesn't work well here
    for colocation in off on; do # -l NUM is the colocation distance, 0 is "don't use it"
      for coocurrence in off on; do # -r enables cooccurrence usae
        for base_word in off on; do # -e enables base word usage
